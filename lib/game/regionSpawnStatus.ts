...
// pokemon_species는 마스터 테이블(select_all, DB.md §11)이라 이름/타입을 같은 쿼리에 임베드해도
// 포획 여부(caught) 판정 자체는 여전히 뷰의 RLS 재적용에만 의존한다.
...
export async function getRegionSpawnStatus(cityId: number): Promise<RegionSpawnStatusResult> {
  ...
  const { data, error } = await supabase
    ...
    .select('dex_no,category,is_legendary,caught,catch_count,pokemon_species(name_kr,type1,type2)')
    .eq('city_id', cityId)
